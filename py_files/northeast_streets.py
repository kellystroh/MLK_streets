...
path = r'/Users/Kelly/galvanize/projects/circles/MLK/northeast/us'
files = glob.glob(path + '/*' + "/*.csv")
states = []
cities = []
for x in files:
    # Replace my path with your own
    x = re.sub('/Users/Kelly/galvanize/projects/circles/MLK/northeast/us/', '', x)
    x = re.sub('/.*', '', x)
    states.append(x)
...
luther_df_NE = pd.DataFrame(columns=['LON', 'LAT', 'NUMBER', 'STREET', 'UNIT', 'CITY', 'DISTRICT', 'REGION','POSTCODE', 'ID', 'HASH'])
...
for csv in range(len(files)):
    ...
    luther_df_NE = pd.concat([luther_df_NE, new, new2], axis=0, ignore_index=True)
    
luther_df_NE.to_csv('luther_df_NE.csv')